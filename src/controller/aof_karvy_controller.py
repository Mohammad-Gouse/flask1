...
def get_file_name(main, sub_folder):
    ...
    #             for file_name in files_in_directory if file_name.startswith(f"AOFKARVY{current_date}_")]
    suffixes = [
        # Extract the numeric part before the extension
        int(file_name.split('_')[-1].split('.')[0])
        for file_name in files_in_directory
        if file_name.startswith(f"AOFKARVY{current_date}_")
    ]
    ...
    file_name = f"AOFKARVY{current_date}_{next_suffix}"
...
QUERY = """
    SELECT row_number() over (order by cm.pan_number) as FIELD1, rta.user_trxn_no as FIELD2, cm.pan_number as FIELD3,
    rta.amc_code as FIELD4, rta.broker_code as FIELD5, rta.broker_code as FIELD6, NULL as FIELD7, NULL as FIELD8,
    rta.client_name as FIELD9, 1 as FIELD10, 'N' as FIELD11, 'OF' as FIELD12,
    'AOF/POA/BR/ASL/PAN/KYC' AS FIELD13,cm.pan_number AS FIELD14,karvy.ih_no AS FIELD15, 'Y' AS FIELD16
    FROM rta_reverse_feed_details rta
    INNER JOIN client_master cm ON cm.client_id = rta.client_id
    INNER JOIN karvy_reverse_field as karvy on karvy.usr_txn_no = rta.user_trxn_no
...
def get_aof_karvy():
    ...
    sub_folder_name = f'karvy/{folder_year}/{folder_month}/{folder_day}'
    ...
    try:
        ...
        if ps_connection:
            ...
            # print(db_response.columns)
            # if len(db_response.columns) < 1:
            #     os.remove(
            #         f'{TRANSACTION_FILE_NAME}/{main_folder_name}/{sub_folder_name}/{file_name}')
