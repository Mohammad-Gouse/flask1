...
# def convert_pdf_to_tiff(input_pdf_path, output_folder, dpi=DPI, max_width=700):
...
# def compress_tiff(input_path, output_path, max_width=700):
#     # Open the TIFF image
...
# def merge_tiff_files(tiff_files, output_tiff_path, max_width=700):
...
from decouple import config
from src.controller.get_tiff_config import get_aof_quality

TRANSACTION_FILE_NAME = Data_Var.data_store_location


# DPI = tiff_config_model.poa_quality_level


def convert_pdf_to_tiff(input_pdf_path, output_folder, dpi, max_width=1000):
    print("poa dpi:", dpi)

    images = convert_from_path(input_pdf_path, dpi)
    # in tiff_files array append each page using for loop and decrease
    tiff_files = []
    for i, image in enumerate(images):
        tiff_path = os.path.join(output_folder, f"page_{i + 1}.tiff")
...
def process_single_pdf(pdf_path, output_folder):
    ...
    tiff_files = convert_pdf_to_tiff(pdf_path, output_folder, DPI)
    return tiff_files
