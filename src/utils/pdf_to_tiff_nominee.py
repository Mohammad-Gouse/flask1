...
def convert_pdf_to_tiff(input_pdf_path, output_folder, dpi=DPI, max_width=1000):
...
def process_single_pdf(pdf_path, output_folder):
    ...
    if pdf_path.lower().endswith(".tiff") or pdf_path.lower().endswith(".tif"):
        ...
        # pages = Image.open(tiff_path)
        print("pdf path", pdf_path)
        print("tiff path: ", tiff_path)
        print("pages: ", pages)

        # print("old page size: ", pages.size)
        # x, y = pages.size
        # x = x - (x*2//100)
        # y = y - (y*2//100)
        # print(x, y)

        # new_pages = pages.resize((x, y))
        # print("new page size: ", new_pages.size)
        tiff_files = []
...
def process_multiple_pdfs(pdf_files, overall_output_folder, file_name):
    from src.utils import colour_pdf_manipulation
    os.makedirs(overall_output_folder, exist_ok=True)
    tiff_files_list = []
    print(pdf_files)
    for pdf_file in pdf_files:
        print("path of pdf file:", pdf_file)
        colour_pdf_manipulation.convertBlack(
            pdf_file, pdf_file)
    for pdf_file in pdf_files:
